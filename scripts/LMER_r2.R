...
uni_lmer <- function(freq_mat){
  ...
  uni_models <- lapply(subtypes, function(subtype){
    ...
    # freq_mat$timepoint <- ifelse(freq_mat$time_point == 'Pre', 0, 1)
    freq_mat$int_cat <- ifelse(freq_mat$int_cat == '<21d', 0, 1)
    freq_mat <- freq_mat |> filter(celltype_r2 == subtype, time_point == 'Pre')
    variables_to_use <- c('freq_r2_comp_scale', 'dataset'[length(unique(freq_mat$dataset)) > 1], 'modality'[length(freq_mat$modality)], 'int_cat'[length(freq_mat$int_cat)])
    variables_to_use <- c(variables_to_use, "(1 | patient)")
    variables_to_use <- variables_to_use[!is.na(variables_to_use)]
...
# Overall
uni_models <- lapply(unique(freq_mat$celltype_r2), function(subtype){
  tryCatch({
    print(subtype)
    df <- filter(freq_mat, celltype_r2 == subtype, time_point == 'Pre')
    df$response <- ifelse(df$response == 'RE', 1, 0)
    variables_to_use <- c('freq_r2_comp_scale', 'modality'[length(unique(df$modality)) > 1], 'int_cat'[length(unique(df$int_cat)) > 1], 'cancertype'[length(unique(df$cancertype)) > 1])
    formula <- as.formula(paste("response ~", paste((variables_to_use), collapse = " + ")))
    ...
    combined_results <- data.frame(
      ...
      Estimate = subtype_results[1],
      StdError = subtype_results[2],
      pValue = coef_table['freq_r2_comp_scale', 'Pr(>|z|)'],
      CI_lower = subtype_confint[1],
      CI_upper = subtype_confint[2]
    )
    return(combined_results)
...
results$fdr <- p.adjust(results$pValue, method = 'fdr', n = nrow(results)) 
celltype_keep <- filter(results, pValue < 0.1) |> pull(Celltypes) |> unique()
results <- filter(results, Celltypes %in% celltype_keep) |> 
  arrange(desc(Estimate)) 
results$fdr_cat <- '<0.05'
results$fdr_cat[results$fdr>0.05] <- '>0.05'
results |> 
  ggplot(aes(x= factor(Celltypes, levels = rev(results$Celltypes)), y=Estimate, ymin=CI_lower, ymax=CI_upper, size = -log10(pValue))) +
  geom_linerange( size=1, position=position_dodge(width = 0.5)) +
  geom_hline(yintercept=0, lty=2) +
  geom_point(shape=21, fill="#0047AB", color='white', stroke = 0.5, position=position_dodge(width = 0.5)) +
  scale_size(breaks = c(-log10(0.001), -log10(0.005), -log10(0.05)), labels = c('<0.001', '<0.005', '<0.05'), range = c(2,5)) +
  # scale_y_continuous(name= "Effect Size", limits = c(-0.3, 0.3)) +
  xlab("") +
  scale_color_manual(values = c("black", "grey")) +
  coord_flip() + theme_minimal() +
