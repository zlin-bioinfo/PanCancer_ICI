...
# Create directory 
directory_name = "/bigdata/zlin/PanCancer_ICI/data/GSE175540/MISTy/"
if not os.path.exists(directory_name):
    os.makedirs(directory_name)
# Load and normalize data
samples = ['c_2','c_34','a_17','b_13','c_20','c_4','b_18','c_3','c_45','c_51','b_1','c_21','a_1','a_15','c_39','b_7','c_23','c_10','c_57','c_7','a_3','c_36','c_5']
dict_adata = {
    sample: sc.read(filename=f'/bigdata/zlin/PanCancer_ICI/data/GSE175540/cell2loc_results/cell2location_map/{sample}/sp.h5ad', library_id=sample)
...
for sample, adata in dict_adata.items():
    ...
    directory_name = "/bigdata/zlin/PanCancer_ICI/data/GSE175540/MISTy/colocalization/"
    if not os.path.exists(directory_name):
        os.makedirs(directory_name)
    misty = genericMistyData(intra=comps)
    misty(model=RandomForestModel, n_jobs=-1, verbose = True)
    misty.uns['target_metrics'].to_csv(f'/bigdata/zlin/PanCancer_ICI/data/GSE175540/MISTy/colocalization/{sample}_target_metrics_RF.csv',index=False)
    misty.uns['interactions'].to_csv(f'/bigdata/zlin/PanCancer_ICI/data/GSE175540/MISTy/colocalization/{sample}_interactions_RF.csv',index=False)
    misty(model=LinearModel, k_cv=10, seed=1000,verbose = True)
    misty.uns['target_metrics'].to_csv(f'/bigdata/zlin/PanCancer_ICI/data/GSE175540/MISTy/colocalization/{sample}_target_metrics_LM.csv',index=False)
    misty.uns['interactions'].to_csv(f'/bigdata/zlin/PanCancer_ICI/data/GSE175540/MISTy/colocalization/{sample}_interactions_LM.csv',index=False)
    # Custom view(PROGENy and TFs)
    directory_name = "/bigdata/zlin/PanCancer_ICI/data/GSE175540/MISTy/PROGENY_TFs/"
    if not os.path.exists(directory_name):
        os.makedirs(directory_name)
    ...
    misty.uns['target_metrics'].to_csv(f'/bigdata/zlin/PanCancer_ICI/data/GSE175540/MISTy/PROGENY_TFs/{sample}_target_metrics.csv',index=False)
    misty.uns['interactions'].to_csv(f'/bigdata/zlin/PanCancer_ICI/data/GSE175540/MISTy/PROGENY_TFs/{sample}_interactions.csv',index=False)
