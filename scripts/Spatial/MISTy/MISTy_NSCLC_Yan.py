...
# Create directory 
directory_name = "/bigdata/zlin/PanCancer_ICI/data/NSCLC_Yan/MISTy/"
if not os.path.exists(directory_name):
    os.makedirs(directory_name)
# Load and normalize data
samples=os.listdir('/bigdata/zlin/PanCancer_ICI/data/NSCLC_Yan/cell2loc_results/cell2location_map/')
dict_adata = {
    sample: sc.read(filename=f'/bigdata/zlin/PanCancer_ICI/data/NSCLC_Yan/cell2loc_results/cell2location_map/{sample}/sp.h5ad', library_id=sample)
...
for sample, adata in dict_adata.items():
    ...
    directory_name = "/bigdata/zlin/PanCancer_ICI/data/NSCLC_Yan/MISTy/colocalization/"
    if not os.path.exists(directory_name):
        os.makedirs(directory_name)
    misty = genericMistyData(intra=comps)
    misty(model=RandomForestModel, n_jobs=-1, verbose = True)
    misty.uns['target_metrics'].to_csv(f'/bigdata/zlin/PanCancer_ICI/data/NSCLC_Yan/MISTy/colocalization/{sample}_target_metrics_RF.csv',index=False)
    misty.uns['interactions'].to_csv(f'/bigdata/zlin/PanCancer_ICI/data/NSCLC_Yan/MISTy/colocalization/{sample}_interactions_RF.csv',index=False)
    misty(model=LinearModel, k_cv=10, seed=1000,verbose = True)
    misty.uns['target_metrics'].to_csv(f'/bigdata/zlin/PanCancer_ICI/data/NSCLC_Yan/MISTy/colocalization/{sample}_target_metrics_LM.csv',index=False)
    misty.uns['interactions'].to_csv(f'/bigdata/zlin/PanCancer_ICI/data/NSCLC_Yan/MISTy/colocalization/{sample}_interactions_LM.csv',index=False)
    # Custom view(PROGENy and TFs)
    directory_name = "/bigdata/zlin/PanCancer_ICI/data/NSCLC_Yan/MISTy/PROGENY_TFs/"
    if not os.path.exists(directory_name):
        os.makedirs(directory_name)
    ...
    misty.uns['target_metrics'].to_csv(f'/bigdata/zlin/PanCancer_ICI/data/NSCLC_Yan/MISTy/PROGENY_TFs/{sample}_target_metrics.csv',index=False)
    misty.uns['interactions'].to_csv(f'/bigdata/zlin/PanCancer_ICI/data/NSCLC_Yan/MISTy/PROGENY_TFs/{sample}_interactions.csv',index=False)
