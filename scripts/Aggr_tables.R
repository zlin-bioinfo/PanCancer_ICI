...
# Load datasets 
datasets <- c('SKCM_Becker', 'BRCA_Bassez1', 'BRCA_Bassez2', 'BCC_Yost', 'CRC_Li', 'CRC_Chen', 'PCa_Hawley', 'TNBC_Shiao', 'TNBC_Zhang', 'HNSC_Franken', 'HNSC_IMCISION', 'HNSC_Luoma', 'SCC_Yost', 'NSCLC_Liu')
...
qsave(list_metadata, '/bigdata/zlin/PanCancer_ICI/tables/meta_list.qs') # save as lists
...
list_metadata <- list_metadata[c('SKCM_Becker', 'BRCA_Bassez1', 'BRCA_Bassez2', 'BCC_Yost', 'CRC_Li', 'CRC_Chen', 'PCa_Hawley', 'TNBC_Shiao', 'TNBC_Zhang', 'HNSC_Franken', 'HNSC_IMCISION', 'HNSC_Luoma', 'SCC_Yost', 'NSCLC_Liu')]
...
rm_pt <- meta_combi |> tabyl(patient, time_point) |> filter(Pre < 200 | On <200) |> pull(patient)
...
write.csv(meta_int, '/bigdata/zlin/PanCancer_ICI/tables/meta_int.csv', row.names = F)
...
# write.csv(meta_filtered, '/bigdata/zlin/PanCancer_ICI/tables/meta_filtered.csv', row.names = F)

# Metadata by sample
meta_sample <- meta_int |> distinct(sample, .keep_all = T)
write.csv(meta_sample, '/bigdata/zlin/PanCancer_ICI/tables/meta_sample.csv', row.names = F)
...
write.csv(meta_patient, '/bigdata/zlin/PanCancer_ICI/tables/meta_patient.csv', row.names = F)
...
# write.csv(mat_roie, '/bigdata/zlin/PanCancer_ICI/tables/roie.csv', row.names = F)
