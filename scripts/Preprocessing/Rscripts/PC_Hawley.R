...
seu <- subset(seu, subset = patient %in% c('Patient1', 'Patient7','Patient10','Patient6'))
seu@meta.data <- seu@meta.data[,!str_detect(colnames(seu@meta.data), 'integrated_snn_res')] 
...
seu$celltype_major[seu$seurat_clusters == '15'] <- 'Plasma cells'
...
seu$celltype_major <- mapvalues(seu$celltype_major,
                                from = c('CD4T','CD8T'),
                                to = c('CD4+ T-cells','CD8+ T-cells'))
# marker_cosg <- cosg(seu |> JoinLayers(), groups='all', assay='RNA', slot='data', mu=1, n_genes_user=100)
DimPlot(seu, group.by = 'celltype_major', cols = getPalette(length(unique(seu$celltype_major))), label = T) /
  DotPlot(seu, group.by = 'celltype_major', features = genes_to_check) + RotatedAxis()
seu <- subset(seu, subset = (celltype_major %in% c('CD4+ T-cells','CD8+ T-cells')) & (seurat_clusters %in% c(4,6,7)), invert = T)
seu <- subset(seu, subset = celltype_major %in% c('unknown','Monocyte','Epithelial','NK','DC','Neutrophils','Melanocytes'), invert = T)
...
library(infercnv)
seu <- qs_read('data/PCa_Hawley/seu_r1.qs2')
if (dir.exists('data/PCa_Hawley/infercnv')==F){
  dir.create('data/PCa_Hawley/infercnv')
}
...
lapply(unique(seu$patient), function(pt){
  ...
  output_dir_full = paste0('data/PCa_Hawley/infercnv/', pt)
...
folders <- c("PCa_Hawley_Patient1", "PCa_Hawley_Patient10", "PCa_Hawley_Patient7")
infercnv_output <- lapply(folders, function(folder){
  print(folder)
  infercnv_obj <- readRDS(paste0('data/PCa_Hawley/infercnv/',folder,'/run.final.infercnv_obj'))
  seu <- add_to_seurat(make_seurat_from_infercnv_obj(infercnv_obj), 
                       infercnv_output_path = paste0('data/PCa_Hawley/infercnv/',folder), assay_name="RNA", top_n=10)
  ...
  ggsave(paste0('data/PCa_Hawley/infercnv/',folder,'/boxplot.pdf'), height = 4, width = 5)
  return(data.frame('Malignant'=seu$malignant))
})
infercnv_output <- do.call(rbind, infercnv_output)
write.csv(infercnv_output, 'data/PCa_Hawley/infercnv/infercnv_output.csv', row.names = T)
