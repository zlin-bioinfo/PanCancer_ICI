#!/usr/bin/env Rscript
rm(list=ls())
pkgs <- c('Seurat','DoubletFinder','scDblFinder','tidyr','plyr','dplyr','stringr','SingleR','scGate','ggsci','BiocParallel','harmony','RColorBrewer','SingleCellExperiment','STACAS','janitor','qs','qs2','COSG')
...
preprocessing <- function(seu, sorted = F, .ncores = 10){
...
# sign_celltype <- read.csv('./tables/celltype_signature.csv') |> as.list() |> lapply(function(x) x[x != ''])
# # noise genes
# gene_rm <- readxl::read_excel('./tables/41586_2022_5400_MOESM3_ESM.xlsx', sheet = '1d_Genes excluded', skip = 2)[,-1]
...
# saveRDS(seu_list, file = './data/SKCM_Becker/raw.rds')
# 
# # Preprocessing
# seu_list <- readRDS('./data/SKCM_Becker/raw.rds')
...
# # qsave(seu_list, file = './data/SKCM_Becker/list.qs')
# # seu_list <- qread('./data/SKCM_Becker/list.qs')
...
# clinical_metadata <- readxl::read_xlsx('./data/SKCM_Becker/clinical_info.xlsx')
...
# # qsave(seu, file = './data/SKCM_Becker/seu_r1.qs')
# # seu <- qread('./data/SKCM_Becker/seu_r1.qs')
# 
...
names(genes_to_check) <- c('T','NK','B','Plasma','pDC','Mast','cDC','Mo/Mac','Neu','Fibro','Muscle','Endo','Mela','Epi','Proliferating')
getPalette <- colorRampPalette(brewer.pal(9, "Set1"))
DimPlot(seu, group.by = 'seurat_clusters', cols = getPalette(length(unique(seu$seurat_clusters))), label = T) /
  DotPlot(seu, group.by = 'seurat_clusters', features = genes_to_check) + RotatedAxis()
DimPlot(seu, group.by = 'celltype_bped_main', cols = getPalette(length(unique(seu$seurat_clusters))), label = T) /
  DotPlot(seu, group.by = 'celltype_bped_main', features = genes_to_check) + RotatedAxis()
table(seu$celltype_bped_main, seu$scGate_multi)
seu$celltype_major <- seu$celltype_bped_main
seu$celltype_major[seu$seurat_clusters == '17'] <- 'pDC'
seu$celltype_major[seu$seurat_clusters == '19'] <- 'Mast'
seu$celltype_major[seu$seurat_clusters == '12'] <- 'Plasma'
seu$celltype_major[seu$seurat_clusters == '21'] <- 'Muscle'
FeaturePlot_scCustom(seurat_object = seu, colors_use = viridis_magma_dark_high, 
...
seu <- CreateSeuratObject(counts = seu@assays$RNA$counts, meta.data = seu@meta.data)
# qsave(seu, file = './data/HNSC_Franken/seu_r1.qs')
# seu <- qread('./data/HNSC_Franken/seu_r1.qs')
...
seu <- CreateSeuratObject(counts = seu@assays$RNA$counts, meta.data = seu@meta.data)
# qsave(seu, file = './data/BRCA_Bassez1/seu_r1.qs')
# seu <- qread('./data/BRCA_Bassez1/seu_r1.qs')
...
DimPlot(seu, label = T) / DotPlot(seu, group.by = 'seurat_clusters', features = genes_to_check) + RotatedAxis() 
DotPlot(seu, group.by = 'celltype_bped_main', features = genes_to_check) + RotatedAxis()
DotPlot(seu, group.by = 'scGate_multi', features = genes_to_check) + RotatedAxis()
table(seu$celltype_bped_main, seu$scGate_multi)
seu$celltype_major <- seu$celltype_bped_main
...
seu$cancertype <- ifelse(seu$subtype == 'HER2+', 'HER2+BC', ifelse(seu$subtype == 'ER+', 'ER+BC', 'TNBC'))
seu <- CreateSeuratObject(counts = seu@assays$RNA$counts, meta.data = seu@meta.data)
# qsave(seu, file = './data/BRCA_Bassez2/seu_r1.qs')
# seu <- qread('./data/BRCA_Bassez2/seu_r1.qs')
...
seu$celltype_bped_fine[is.na(seu$celltype_bped_fine)] <- 'unknown'
seu$scGate_multi[is.na(seu$scGate_multi)] <- 'unknown'
# qsave(seu, file = './data/TNBC_Zhang/processing.qs')
# seu <- qread('./data/TNBC_Zhang/processing.qs')
qs_save(seu, file = './data/TNBC_Zhang/processing.qs2')
...
seu <- CreateSeuratObject(counts = seu@assays$RNA$counts, meta.data = seu@meta.data)
# qsave(seu, file = './data/TNBC_Zhang/seu_r1.qs')
# seu <- qread('./data/TNBC_Zhang/seu_r1.qs')
qs_save(seu, './data/TNBC_Zhang/seu_r1.qs2')
...
seu$celltype_bped_fine[is.na(seu$celltype_bped_fine)] <- 'unknown'
seu$scGate_multi[is.na(seu$scGate_multi)] <- 'unknown'
# qsave(seu, file = './data/BCC_Yost/processing.qs')
# seu <- qread('./data/BCC_Yost/processing.qs')
qs_save(seu, './data/BCC_Yost/processing.qs2')
seu <- qs_read('./data/BCC_Yost/processing.qs2')
tcr <- read.table('./data/BCC_Yost/GSE123813_bcc_tcr.txt')
...
seu <- CreateSeuratObject(counts = seu@assays$RNA$counts, meta.data = seu@meta.data)
# qsave(seu, file = './data/BCC_Yost/seu_r1.qs')
# seu <- qread('./data/BCC_Yost/seu_r1.qs')
...
tcr <- read.table('./data/SCC_Yost/GSE123813_scc_tcr.txt')
seu[['cdr3s_nt']] <- tcr$cdr3s_nt[match(rownames(seu@meta.data), rownames(tcr))]
...
seu$modality <- 'Mono'
seu <- CreateSeuratObject(counts = seu@assays$RNA$counts, meta.data = seu@meta.data)
# qsave(seu, file = './data/SCC_Yost/seu_r1.qs')
seu <- qread('./data/SCC_Yost/seu_r1.qs')
qs_save(seu, './data/SCC_Yost/seu_r1.qs2')
...
seu <- CreateSeuratObject(counts = seu@assays$RNA$counts, meta.data = seu@meta.data)
# qsave(seu, file = './data/NSCLC_Liu/seu_r1.qs')
# seu <- qread('./data/NSCLC_Liu/seu_r1.qs')
qs_save(seu, './data/NSCLC_Liu/seu_r1.qs2')
...
seu_list <- lapply(seu_list, function(seu){
  ...
  seu[['singlet']] <- ifelse(seu$scDblFinder.class=='doublet' & seu$DoubletFinder.class=='Doublet', 'no', 'yes')
  # filter low-quality cells and doublets
  seu <- subset(seu, subset = nFeature_RNA > 400 & nFeature_RNA < 6000 & percent_mito < 20 & singlet =='yes')
  seu <- StandardizeGeneSymbols(seu, slot = 'counts', EnsemblGeneTable=EnsemblGeneTable.Hs)
  seu <- seu%>%
    NormalizeData() |>
    FindVariableFeatures() |> 
...
# qsave(seu_list, file = './data/CRC_Li/list.qs')
# seu_list <- qread('./data/CRC_Li/list.qs')
qs_save(seu_list, './data/CRC_Li/list.qs2')
seu_list <- qread('./data/CRC_Li/list.qs2')
# check sample quality
(df <- data.frame(
  nfeature = sapply(seu_list, function(seu){nfeature <- sum(Matrix::rowSums(seu@assays$RNA$counts>0)>3)
  return(nfeature)}),
...
seu <- CreateSeuratObject(counts = seu@assays$RNA$counts, meta.data = seu@meta.data)
qsave(seu, file = './data/CRC_Li/seu_r1.qs')
seu <- qread('./data/CRC_Li/seu_r1.qs')
qs_save(seu, file = './data/CRC_Li/seu_r1.qs2')
seu <- qread('./data/CRC_Li/seu_r1.qs2')
# IMCISION
matrix_count <- read.table('./data/HNSC_IMCISION/GSM7324294_Count_data_IMCISION.txt') |> as.sparse()
matrix_meta <- read.table('./data/HNSC_IMCISION/GSM7324295_Meta_data_IMCISION.txt', header = TRUE, sep = '\t')
seu <- CreateSeuratObject(matrix_count, meta.data = matrix_meta)
...
seu <- CreateSeuratObject(counts = seu@assays$RNA$counts, meta.data = seu@meta.data)
# qsave(seu, file = './data/HNSC_IMCISION/seu_r1.qs')
# seu <- qread('./data/HNSC_IMCISION/seu_r1.qs')
qs_save(seu, './data/HNSC_IMCISION/seu_r1.qs2')
seu <- qs_read('./data/HNSC_IMCISION/seu_r1.qs2')
#HNSC Luoma
...
# qsave(seu_list, file = './data/HNSC_Luoma/list.qs')
# seu_list <- qread('./data/HNSC_Luoma/list.qs')
qs_save(seu_list, file = './data/HNSC_Luoma/list.qs2')
# # check sample quality
# df <- data.frame(
#   nfeature = sapply(seu_list, function(seu_obj){nfeature <- sum(Matrix::rowSums(seu_obj@assays$RNA@counts>0)>3)
#   return(nfeature)}),
...
# contig_list <- purrr::map(paste0('./data/HNSC_Luoma/' ,c(files_tcr_pre, files_tcr_post)), read.csv)
...
seu <- CreateSeuratObject(counts = seu@assays$RNA$counts, meta.data = seu@meta.data)
qsave(seu, file = './data/HNSC_Luoma/seu_r1.qs')
seu <- qread('./data/HNSC_Luoma/seu_r1.qs')
...
DimPlot(seu, group.by = 'celltype_major', cols = getPalette(length(unique(seu$seurat_clusters)))) /
  DotPlot(seu, group.by = 'celltype_major', features = genes_to_check) + RotatedAxis()
seu <- subset(seu, subset = (celltype_major %in% c('unknown','Neutrophils')) | (scGate_multi == 'Multi'), invert = T)
# qsave(seu, file = './data/PCa_Hawley/seu_r1.qs')
# seu <- qread('./data/PCa_Hawley/seu_r1.qs')
...
# sce <- dior::read_h5(file = "./data/TNBC_Shiao/adata_cd45n.h5", target.object = "singlecellexperiment")
# metadata <- read.csv(file = "./data/TNBC_Shiao/obs_cd45n.csv", row.names = 1) |> select(batch, cohort, treatment, response_group)
# seu <- CreateSeuratObject(counts = assay(sce, "X"), meta.data = metadata)
# qsave(seu, file = "./data/TNBC_Shiao/seu_cd45n.qs")
# CD45+
# sce <- dior::read_h5(file = "./data/TNBC_Shiao/adata_cd45p.h5", target.object = "singlecellexperiment")
# metadata <- read.csv(file = "./data/TNBC_Shiao/obs_cd45p.csv", row.names = 1) |> select(batch, CD45_enrich, batch_num, cohort, pCR, cleared_nodes, treatment, patient_treatment, combined_tcr)
# seu <- CreateSeuratObject(counts = assay(sce, "X"), meta.data = metadata)
# qsave(seu, file = "./data/TNBC_Shiao/seu_cd45p.qs")
...
# remove samples with <200 cells or <9000 genes 
sample_rm <- rownames(filter(df, nfeature < 9000 | ncell < 200)); sample_rm
seu <- subset(seu, subset = patient %in% c("Patient03T1", "Patient03T2", "Patient30", "Patient63"), invert = T)
...
seu$celltype_bped_fine[is.na(seu$celltype_bped_fine)] <- 'unknown'
seu$scGate_multi[is.na(seu$scGate_multi)] <- 'unknown'
qs_save(seu, './data/TNBC_Shiao/seu_r1_processing.qs2')
seu <- qs_read('./data/TNBC_Shiao/seu_r1_processing.qs2')
genes_to_check = list(c('CD3D', 'CD3E', 'CD4', 'CD8A', 'CD8B'), # T cells 'CD8B'
                      c('KLRD1','KLRB1', 'KLRC1', 'NCAM1'), # NK cells 'KLRB1', 'KLRC1', 'CD16', 'CD56', 'CD11b', 'CD11c'
                      c('CD79A','CD19', 'MS4A1'),  # B cells 
                      c('CD27','CD38','JCHAIN'), # Plasma cells 
...
names(genes_to_check) <- c('T','NK','B','Plasma','pDC','Mast','cDC','Mo/Mac','Neutro','Fibro','Endo','Mela','Epi','Proliferating')
...
seu <- CreateSeuratObject(counts = seu@assays$RNA$counts, meta.data = seu@meta.data)
qsave(seu, file = './data/TNBC_Shiao/seu_r1.qs')
seu <- qread('./data/TNBC_Shiao/seu_r1.qs')
...
seu <- seu |> 
  ...
  RunUMAP(reduction = "harmony", dims = 1:20) |>  
  FindNeighbors(reduction = "harmony", dims = 1:20) |> FindClusters()
qsave(seu, './data/CRC_Chen/processing.qs')
seu <- qread('./data/CRC_Chen/processing.qs')
genes_to_check = list(c('CD3D', 'CD3E', 'CD4', 'CD8A', 'CD8B'), # T cells 'CD8B'
                      c('KLRD1','KLRB1', 'KLRC1', 'NCAM1'), # NK cells 'KLRB1', 'KLRC1', 'CD16', 'CD56', 'CD11b', 'CD11c'
                      c('CD79A','CD19', 'MS4A1'),  # B cells 
                      c('CD27','CD38','JCHAIN'), # Plasma cells 
...
qsave(seu, file = './data/CRC_Chen/seu_r1.qs')
...
seu$percent_mito <- PercentageFeatureSet(seu, pattern = "^MT-")
seu$percent_ribo <- PercentageFeatureSet(seu, pattern = "^RP[SL]")
VlnPlot(seu, group.by = 'sample', features = 'percent_mito')

seu_list <- SplitObject(seu, split.by = 'sample')
seu_list <- 
seu_list <- qread('./data/SKCM_PLozniak/seu_list.qs')
seu_list <- lapply(seu_list, function(seu){
...
seu$celltype_major[seu$scGate_multi == 'Multi'] <- 'Multi'
seu$celltype_major[seu$seurat_clusters == 15] <- 'pDC'
seu$celltype_major[seu$seurat_clusters %in% c(0,4,7,12,16)] <- 'Malignant cells'
seu$celltype_major[seu$seurat_clusters == 13] <- 'Epithelial cells'
DotPlot(seu, group.by = 'celltype_major', features = genes_to_check) + RotatedAxis()
DimPlot(seu, group.by = 'celltype_major', cols = getPalette(length(unique(seu$seurat_clusters)))) /
  DotPlot(seu, group.by = 'celltype_major', features = genes_to_check) + RotatedAxis()
table(seu$celltype_major)
seu <- subset(seu, subset = (celltype_major %in% c('unknown','Melanocytes','Neutrophils','Multi','Malignant cells','Epithelial cells')), invert = T)
seu <- seu |> JoinLayers()
seu <- CreateSeuratObject(counts = GetAssayData(seu, assay = 'RNA', slot = 'counts'), meta.data = seu@meta.data)
qsave(seu, file = './data/SKCM_Plozniak/seu_r1.qs')
