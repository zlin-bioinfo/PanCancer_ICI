...
if (dir.exists('data/CRC_Li/infercnv')==F){
  dir.create('data/CRC_Li/infercnv')
}
...
lapply(unique(seu$patient), function(pt){
  ...
  output_dir_full = paste0('data/CRC_Li/infercnv/', pt)
...
folders <- list.files('data/CRC_Li/infercnv')
infercnv_output <- lapply(folders, function(folder){
  print(folder)
  infercnv_obj <- readRDS(paste0('data/CRC_Li/infercnv/',folder,'/run.final.infercnv_obj'))
  seu <- add_to_seurat(make_seurat_from_infercnv_obj(infercnv_obj), 
                       infercnv_output_path = paste0('data/CRC_Li/infercnv/',folder), assay_name="RNA", top_n=10)
  ...
  ggsave(paste0('data/CRC_Li/infercnv/',folder,'/boxplot.pdf'), height = 4, width = 5)
  return(data.frame('Malignant'=seu$malignant))
})
infercnv_output <- do.call(rbind, infercnv_output)
write.csv(infercnv_output, 'data/CRC_Li/infercnv/infercnv_output.csv', row.names = T)
